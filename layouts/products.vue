<template>
  <div>
    ...
    <nav class="container mx-auto p-4 flex justify-between items-center">
      <NuxtLink to="/products" class="font-bold text-white">Vue Exercise Merch</NuxtLink>
...
const { cartlist, categories } = storeToRefs(store); // Retrieve categories from the store
const drawer = ref(false); // Drawer state
const checkoutModal = ref(null); // Reference to the CheckoutModal
...
onMounted(() => {
  store.fetchCategories(); // Fetch categories when the component mounts
});
</script>
